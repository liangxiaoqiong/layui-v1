...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		//p(\$ids);
		\$where = array('article.status' => 1, 'article.cid'=> array('IN',\$ids));
	}else {
	...
	\$type = Common\Lib\Category::getSelf(get_category(0), \$artlist['cid']);		
	\$artlist['curl'] = get_url(\$type);
	\$artlist['comment'] = get_comment(\$artlist['id'],\$artlist['modelid']);
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('product.status' => 1, 'product.cid'=> array('IN',\$ids));
	}else {
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('picture.status' => 1, 'picture.cid'=> array('IN',\$ids));
	}else {
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('soft.status' => 1, 'soft.cid'=> array('IN',\$ids));
	}else {
...
str;
    ...
    public function _list($attr, $content)
    {
        ...
        $pageroll  = empty($attr['pageroll']) ? '5' : $attr['pageroll'];
        $pagetheme = empty($attr['pagetheme']) ? ' %HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%' : htmlspecialchars_decode($attr['pagetheme']);
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array(\$_tablename.'.status' => 1, \$_tablename .'.cid'=> array('IN',\$ids));
	}else {
		\$_tablename = 'article';
		\$where = array(\$_tablename.'.status' => 1);
	...
	\$type = Common\Lib\Category::getSelf(get_category(0), \$list['cid']);		
	\$list['curl'] = get_url(\$type);
	\$list['comment'] = get_comment(\$list['id'],\$list['modelid']);
...
str;
    ...
    //手机通用列表
    public function _mlist($attr, $content)
    {
        $flag     = empty($attr['flag']) ? '' : $attr['flag'];
        $typeid   = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //只接收一个栏目ID
        $titlelen = empty($attr['titlelen']) ? 0 : intval($attr['titlelen']);
...
<?php
	...
	if (!empty(\$_tablename) && \$_tablename != 'page') {
		...
		\$_mlist = D2('ArcView',"\$_tablename")->nofield('$nofield')->where(\$where)->order("$orderby")->limit(\$limit)->select();
		if (empty(\$_mlist)) {
			\$_mlist = array();
		}
	}else {
	...
	foreach(\$_mlist as \$autoindex => \$mlist):

	\$_jumpflag = (\$mlist['flag'] & B_JUMP) == B_JUMP? true : false;
	\$mlist['url'] = get_content_url(\$mlist['id'], \$mlist['cid'], \$mlist['ename'], \$_jumpflag, \$mlist['jumpurl']);
	
	\$type = Common\Lib\Category::getSelf(get_category(0), \$mlist['cid']);		
	\$mlist['curl'] = get_url(\$type);
	\$mlist['comment'] = get_comment(\$mlist['id'],\$mlist['modelid']);
	
	if($titlelen) \$mlist['title'] = str2sub(\$mlist['title'], $titlelen, 0);
	if($infolen) \$mlist['description'] = str2sub(\$mlist['description'], $infolen, 0);
    if(isset(\$mlist['pictureurls'])) \$mlist['pictureurls'] = get_picture_array(\$mlist['pictureurls']);
...
<?php
	...
	if (\$_typeid>0) {
		...
		if (\$_selfcate) {
			...
			\$where = array('special.status' => 1, 'special.cid'=> array('IN',\$ids));
		}else {
			\$_typeid = 0;
		}
	}
...
str;
    ...
    //论坛列表
    public function _forumlist($attr, $content)
    {
        ////非debug参属性参数只处理 一次
        $flag     = empty($attr['flag']) ? '' : $attr['flag'];
        $typeid   = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //-1后面自动获取
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('forum.status' => 1, 'forum.cid'=> array('IN',\$ids));
	}else {
		\$where = array('forum.status' => 1);
	}

	if (\$_keyword != '') {
		\$where['forum.title'] = array('like','%'.\$_keyword.'%');
	}
	if (!empty(\$_arcid)) {
		\$where['forum.id'] = array('IN', \$_arcid);
	}

	if ($flag > 0) {
		\$where['_string'] = 'forum.flag & $flag = $flag ';
	}
	...
	\$_forumlist = D2('ArcView','forum')->nofield('$nofield')->where(\$where)->order("$orderby")->limit(\$limit)->select();
	...
	foreach(\$_forumlist as \$autoindex => \$forumlist):

	\$_jumpflag = (\$forumlist['flag'] & B_JUMP) == B_JUMP? true : false;
	\$forumlist['url'] = get_content_url(\$forumlist['id'], \$forumlist['cid'], \$forumlist['ename'], \$_jumpflag, \$forumlist['jumpurl']);
	
	\$type = Common\Lib\Category::getSelf(get_category(0), \$forumlist['cid']);		
	\$forumlist['curl'] = get_url(\$type);
	\$forumlist['reply'] = get_forum_reply(\$forumlist['id']);

	if($titlelen) \$forumlist['title'] = str2sub(\$forumlist['title'], $titlelen, 0);
	if($infolen) \$forumlist['description'] = str2sub(\$forumlist['description'], $infolen, 0);
...
str;
    ...
    //导航
    public function _catin($attr, $content)
    {
        $typeid  = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //接收多个栏目ID
        $type    = empty($attr['type']) ? 'son' : $attr['type']; //son表示下级栏目,self表示同级栏目,top顶级栏目(top忽略typeid)
        $flag    = empty($attr['flag']) ? 0 : intval($attr['flag']); //0(不显示链接和单页),1(全部显示),2
        $orderby = empty($attr['orderby']) ? 'id DESC' : $attr['orderby'];
        $limit   = empty($attr['limit']) ? '10' : $attr['limit'];
...
<?php
	...
	if(\$_typeid == '-1'){
		\$_typeid = I('cid', 0, 'intval');
		\$__catin = get_category(1);
	}else{
		\$__catin = get_category1(1,\$_typeid);
	}
	
	if ($modelid) {
		\$__catin = Common\Lib\Category::getLevelOfModelId(\$__catin, $modelid);
	}


	if ($flag == 0) {
		//where array('status' => 1, 'type' => 0 , 'modelid' => array('neq',2));//2是单页模型
		\$__catin = Common\Lib\Category::clearPageAndLink(\$__catin);
	}
	
	//\$type为top,忽略$typeid
	if(\$_typeid == 0 || \$_type == 'top') {
		\$_catin  = Common\Lib\Category::toLayer(\$__catin);
	}else {
		//son，子类列表
		\$_catin  = Common\Lib\Category::toLayer(\$__catin, 'child', 0);
	}
	foreach(\$_catin as \$autoindex => \$catin):
	if(\$autoindex < \$_limit[0]) continue;
	if(\$autoindex >= (\$_limit[1]+\$_limit[0])) break;
	\$catin['url'] = get_url(\$catin);
?>
str;
...
str;
    ...
    //union
    public function _union($attr, $content)
    {
        $id   = !isset($attr['id']) || $attr['id'] == '' ? 0 : trim($attr['id']);
        $flag = empty($attr['flag']) ? '0' : $attr['flag'];
...
<?php
	...
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['tablename']) ) {
		...
		//上一条记录
        \$_vo= D2('ArcView', \$cate['tablename'])->where(array(\$cate['tablename'].'.status' => 1, 'cid' => \$content['cid'], 'id' => array('lt',\$content['id'])))->order('id desc')->find();
        ...
        if (\$_vo) {
			...
			echo '<a href="'. \$_vo['url'] .'"$target" title="'.\$_vo['title'].'">'. \$_vo['title'] .'</a>';
        } else {
        	echo "$notitle";
        }
	}
...
<?php
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['tablename']) ) {
		...
		//下一条记录
        \$_vo= D2('ArcView',\$cate['tablename'])->where(array(\$cate['tablename'].'.status' => 1, 'cid' => \$content['cid'], 'id' => array('gt',\$content['id'])))->order('id ASC')->find();
